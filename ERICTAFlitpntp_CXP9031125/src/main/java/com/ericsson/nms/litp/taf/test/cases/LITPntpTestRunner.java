...
public class LITPntpTestRunner extends TorTestCaseHelper {
    
    Logger logger = Logger.getLogger(LITPntpTestRunner.class);
    ...
    @TestId(id = "CXP9031125-1", title = "Upgrade ERIClitpntp rpm on the MS")
    @Test(groups={"ACCEPTANCE"})
    public void upgradeERIClitpntpRPM() throws TimeoutException, FileNotFoundException {

        rpmUpgradeOperator.initialise("ntp");
     ...
     * @PRIORITY HIGH
     */
    @TestId(id = "CXP9031125-2", title = "Run python test cases for ERIClitpntp package")
    @Test(groups={"CDB_REGRESSION", "ACCEPTANCE"})
    public void runERIClitpntpTests() {
     ...
     * @PRE Execution of test {@link #runERIClitpntpTests()}
     * @PRIORITY HIGH
     * @param className
     * @param name
     * @param failures
     */
    @TestId(id = "CXP9031125-3", title = "Parse xml outputs from python tests")
